...
# Copiammos las funcioines de las clases originales actualmente en timm.layers
# ya que se ha deprecado el timm.models.layers

import warnings

"""
Código de la clase original de timm para no tener que depender de la libreria (timm/models/layers/drop.py)

"""
# Esta función ya no existe actualmente en timm, está deprecada y se usa _ntuple pero a nosotros nos viene bien así
# helpers.py -> from timm.model.layers import to_2tuple
def to_2tuple(x):
...
###############################################################################
# weight_init.py -> from timm.layers import trunc_normal_
...
# drop.py -> from timm.models.layers import DropPath
...
#################################################################
# Continuación de la clase original
...
class MaskPredictor(nn.Module):
    ...
    def forward(self, q, k, token_mask=None):
        ...
        """
        CAMBIO 2: Adaptación a longitud variable
        """
        # ANTES
        # assert self.num_tokens == N  <-- Esto crasheaba si el tamaño no era exacto
        # AHORA: Permitimos N <= num_tokens, pero lanzamos error si es mayor
        ...
        # Enmascarar padding si existe en la entrada
        if token_mask is not None:
        ...
        """
        Slicing Dinámico : Parte de compresión
        """
        # ANTES: k = k @ self.proj_n  <-- Error de dimensiones si N < Max
        # (... x 1500) x (4096 x ---)  --> Daría ERROR por incompatibilidad de tamaños
        # AHORA: Recortamos la matriz de proyección al tamaño actual N
        proj_n_sliced = self.proj_n[:N, :]  # Nos quedamos con las N primeras filas tal que:
                                            # (... x 1500) x (1500 x ...) -> Encajan perfectamente  
        k = k @ proj_n_sliced   # Resultado: K comprimida [B, H, c, k_dim]

        """
        Slicing Dinámico : Parte de Reconstrucción
        """
        # Recortamos la matriz inversa (basis) para poder descomprimir luego
        basis = self.proj_back_n[:N, :].permute(1, 0) # Gira la matriz con 0 FILTROS
        
        # Si no estamos entrenando la reconstrucción, aplicamos el threshold que habiamos establecido
        # Aplicamos valor absoluto para juzgar magnitud, no signo
        if not (self.training and self.cfg.LOSS.USE_ATTN_RECON):
            basis = self.basis_threshold(basis.abs())           
        
        # -----------------------------------------------------------------------
        # CÁLCULO DE ATENCIÓN DE BAJO RANGO ("Cheap Attention")
        # -----------------------------------------------------------------------
        # En lugar de multiplicar (N x N), multiplicamos (N x k_dim).
        # Esto nos da un "mapa de calor borroso" de dónde mirar.
        cheap_attn = (q @ k) * self.scale  # [B, H, N, K_dim]
        cheap_attn = cheap_attn[..., 1:, :]  # [B, H, N-1, K] Ignoramos el token CLS para el cálculo
        basis_coef = cheap_attn.softmax(dim=-1) # Coeficientes de importancia
        
        
        # -----------------------------------------------------------------------
        # RECONSTRUCCIÓN DE LA MATRIZ DE ATENCIÓN (N x N)
        # -----------------------------------------------------------------------
        if self.training and self.cfg.LOSS.USE_ATTN_RECON:
            # Entrenamiento suave, usamos todo para que fluyan los gradientes
            approx_attn = basis_coef @ basis
        else:
            # En inferencia o entrenamiento sparse, forzamos ceros antes de multiplicar
            # Con esto ahorramos cómputo al multiplicar matrices dispersas
            if cfg.BASIS_COEF.USE_TOPK:
                # Nos quedamos solo con los K coeficientes más altos
                ...
                # Si no, aplicamos un umbral
                basis_coef = self.basis_coef_threshold(basis_coef)

            # Multiplicamos: (Coeficientes Dispersos) @ (Base Dispersa) = Atención Aproximada
            approx_attn = basis_coef @ basis

        # Limpiar columnas inactivas si hay padding
        attn_score = approx_attn.clone()
        if token_mask is not None:
            # Ponemos -infinito para que el softmax posterior sea 0 absoluto
            attn_score[..., 1:].masked_fill_(~token_mask[:, None, None, :], float('-inf'))


        """ 
        CAMBIO 3: Presupuesto de Atención Dinámico (Generación de Máscara Final)
        """
        if cfg.ATTN_SCORE.USE_TOPK:
            # ANTES: topk con self.attn_budget fijo -> Error si audio es corto.
            # AHORA: Ajustamos K al tamaño real del audio.
            real_k = min(self.attn_budget, N) 
            
            # Seleccionamos los índices finales donde SÍ calcularemos atención real
        ...
        # Limpiar filas inactivas (si el origen es padding, no atiende a nadie)
        if token_mask is not None and cfg.PRUNE_ATTN_MATRIX_ROW:
            attn_mask *= token_mask[:, None, :, None]

        # Reintegrar token CLS
        # El Sparsifiner ignora el token CLS en el cálculo, aquí lo añadimos de vuelta
        # para que sea visible en la máscara final
        ...
        # Métrica de dispersión (solo para logs/vailidación)
